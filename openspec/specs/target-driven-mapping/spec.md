...
### Requirement: Chain-of-thought streaming interface
The mapping agent SHALL expose a generator-based streaming interface (`map_source_to_target_stream` / `map_source_to_bird_stream`) that yields structured progress events during execution, enabling real-time UI feedback.

#### Scenario: Streaming generator available alongside sync wrapper
- **WHEN** a caller needs real-time progress
- **THEN** the caller SHALL use `map_source_to_target_stream()` which yields `MappingEvent` dicts per processing step
- **AND** the synchronous `map_source_to_target()` SHALL remain available as a thin wrapper that exhausts the generator and returns the final mapping

#### Scenario: Source pre-filtering returns scored candidates
- **WHEN** the agent pre-filters source tables for a target table
- **THEN** the internal `_select_source_tables()` SHALL return both the filtered columns and the scored candidate table names (with scores) so downstream consumers can report which candidates were considered
